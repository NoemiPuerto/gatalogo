...
export async function PATCH(_request, { params }) {
  ...
  const adoptionRequest = await prisma.adoptionRequest.update({ where: { id }, data: { status: "REJECTED" } });
  await prisma.notification.create({ data: { userId: existing.userId, title: "Adoption request rejected", message: `Your request for ${existing.cat.name} was rejected.` } });
